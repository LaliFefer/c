...
internal class SaleImplementation : ISale
{
    ...
    public Sale? Read(int id)
    {
        ...
        return Sales.FirstOrDefault(s => s?.IDNumber == id);
    }

    // 3. קריאת כל המכירות
    public List<Sale> ReadAll()
    {
        // החזרת עותק של הרשימה (כדי למנוע שינויים לא רצויים מבחוץ)
    ...
    public void Update(Sale item)
    {
        ...
        if (existing == null)
            throw new Exception($"Sale with ID {item.IDNumber} does not exist.");

        // הסרת הישן והוספת המעודכן
